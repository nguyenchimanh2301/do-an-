    ...
    class Formhoadon
    {
        ...
        public void timhd()
        {
            ...
            if (i < list.Count)
            {
                ...
                gd.pain("MÃ PHÒNG", 1, 13); gd.pain("SỐ ĐIỆN(kw)", 12, 13); gd.pain("SỐ NƯỚC(m/3)", 24, 13); gd.pain("TG thuê(tháng)", 37, 13); gd.pain("$ VỆ SINH(VND)", 52, 13); gd.pain("$ KHÁC(VND)", 68, 13); gd.pain("TỔNG TIỀN(VND)", 82, 13);
                Console.WriteLine(list[i].Maph + "\t\t" + list[i].Soswdien + "\t\t" + list[i].Sonuoc + "\t" + list[i].Tgiano + "\t\t" + list[i].Tienvesinh + "\t\t" + list[i].Tkhac + "\t\t" + list[i].Tt);
            }
            else
            {
                Console.WriteLine("không tồn tại mã hóa đơn");
